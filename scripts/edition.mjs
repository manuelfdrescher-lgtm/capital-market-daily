...
11. Als editor die finale Ausgabe schreiben: Leitartikel und große Meldungen mit Artikelbild (image-Block; gestochene SVG-Tafeln im Markenstil unter public/images/ anlegen oder passende vorhandene wiederverwenden, nur lokale Dateien). JSON nach content/editions/${slug}.json (edition.number = ${nextNumber}, edition.date = "${date}", edition.slot = "${slot}", edition.isExample = false, edition.generatedAt = aktuelle ISO-Zeit, edition.dataAsOf = ehrliche Beschreibung des Datenstands).
